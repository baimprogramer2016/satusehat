...
class DiagnosticReportJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $data_data = $this->diagnostic_report_repo->getDataDiagnosticReportReadyJob();
        ...
        if ($data_data->count() > 0) {
            ...
            foreach ($data_data as $item_data) {
                ...
                # API POST Bundle
                $payload_diagnostic_report = $this->bodyManualDiagnosticReport($item_data, $data_parameter);
                ...
                if ($response->successful()) {
                    ...
                    if ($body_parse->resourceType == 'OperationOutcome') {
                        ...
                        $this->diagnostic_report_repo->updateStatusDiagnosticReport($id, $satusehat_id, $payload_diagnostic_report, $response);
                    }
                } else {
                    $this->diagnostic_report_repo->updateStatusDiagnosticReport($id, $satusehat_id, $payload_diagnostic_report, $response);
                }
