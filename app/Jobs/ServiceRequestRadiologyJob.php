...
class ServiceRequestRadiologyJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $data_data = $this->service_request_repo->getDataServiceRequestRadiologyReadyJob();
        ...
        if ($data_data->count() > 0) {
            ...
            foreach ($data_data as $item_data) {
                ...
                # API POST Bundle
                $payload_service_request = $this->bodyManualServiceRequestRadiology($item_data, $data_parameter);
                ...
                if ($response->successful()) {
                    ...
                    if ($body_parse->resourceType == 'OperationOutcome') {
                        ...
                        $this->service_request_repo->updateStatusServiceRequestRadiology($id, $satusehat_id, $payload_service_request, $response);
                    }
                } else {
                    $this->service_request_repo->updateStatusServiceRequestRadiology($id, $satusehat_id, $payload_service_request, $response);
                }
