...
use App\Jobs\CatatanPengobatanJob;
use App\Jobs\MedicationDispenseJob;
use App\Repositories\CatatanPengobatan\CatatanPengobatanInterface;
...
class CatatanPengobatanController extends Controller
{
    use GeneralTrait;
    use ApiTrait;
    use JsonTrait;

    private $catatan_pengobatan_repo, $medication_request_repo, $parameter_repo;
    public $job_logs_repo;
    protected $job_id = 0;
    public function __construct(
        JobLogsInterface $jobLogsInterface,
        CatatanPengobatanInterface $catatanPengobatanInterface,
        ParameterInterface $parameterInterface,
        MedicationRequestInterface $medicationRequestInterface

    ) {
        $this->job_logs_repo = $jobLogsInterface;
        $this->catatan_pengobatan_repo = $catatanPengobatanInterface;
    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = $this->catatan_pengobatan_repo->getQuery($request->all());
            return Datatables::of($data)
                ...
                ->addColumn('action', function ($item_medication_dispense) {
                    ...
                    if ($item_medication_dispense->satusehat_send_catatan_pengobatan == 1) {
                        $li_kirim_ss = '';
                        $li_response_ss = "<li><a href='#file-upload' data-toggle='modal' onClick=modalResponseSS('" . $this->enc($item_medication_dispense->satusehat_id_catatan_pengobatan) . "')><em class='icon ni ni-eye'></em><span>Response Satu Sehat</span></a></li>";
                    } else {
        ...
        return view("pages.catatan-pengobatan.catatan-pengobatan");
    }

    public function responseSS(Request $request, $id)
    {
        try {
            $response_satusehat  = $this->api_response_ss('/MedicationStatement', $id);
            return view('pages.catatan-pengobatan.catatan-pengobatan-response-ss', [
    ...
    {
        try {
            return view('pages.catatan-pengobatan.catatan-pengobatan-kirim-ss', [
                "data_medication_dispense" => $this->catatan_pengobatan_repo->getDataCatatanPengobatanFind($this->dec($id)),
            ]);
        } catch (Throwable $e) {
            return view("layouts.error", [
                "message" => $e
    ...
    public function kirimSS(Request $request)
    {
        try {
            ...
            $data_medication_dispense = $this->catatan_pengobatan_repo->getDataCatatanPengobatanFind($this->dec($request->id));
            ...
            if (empty($data_medication_request['r_encounter']['satusehat_id'])) {
                ...
                if ($response->successful()) {
                    ...
                    if ($body_parse->resourceType == 'OperationOutcome') {
                        ...
                        $this->catatan_pengobatan_repo->updateStatusCatatanPengobatan($this->dec($request->id), $satusehat_id, $payload_medication_dispense, $response);
                    }
                }
    ...
    public function runJob(Request $request, $param_id_jadwal)
    {
        try {
            ...
            // $data_medication_dispense = $this->catatan_pengobatan_repo->getDataCatatanPengobatanFind(4521);
            // $data_medication_request = $this->medication_request_repo->getDataMedicationRequestIdentifier($data_medication_dispense['encounter_original_code'], $data_medication_dispense['identifier_1'], $data_medication_dispense['identifier_2']);
            // $data_parameter = $this->parameter_repo->getDataParameterFirst();

            // return $this->bodyManualCatatanPengobatan($data_medication_dispense, $data_medication_request, $data_parameter);
            ...
            if ($this->catatan_pengobatan_repo->getDataCatatanPengobatanReadyJob()->count() > 0) {
                ...
                if ($this->job_logs_repo->getDataJobLogAlreadyRun($param_start['id']) > 0) {
                    ...
                    CatatanPengobatanJob::dispatch(
                        $this->parameter_repo,
                        $this->job_logs_repo,
                        $this->job_id,
                        $this->catatan_pengobatan_repo,
