...
<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">Prognosis Master</h3>
...
<script type="text/javascript">
    ...
    {
        var id_input_element = event.target.name.replace('id_prognosis_master_', '');


        // console.log(display_name)
        loadingProcess(); //dari custom.js
        var url     ='{{ route("prognosis-master-kode",":id") }}';
    ...
    function updateKode(id_prognosis_master,kode_prognosis)
    {
        var inputElement = document.querySelector('input[name="id_prognosis_master_'+id_prognosis_master+'"]');


        var param = {
            id_prognosis_master : parseInt(id_prognosis_master),
            kode_prognosis : kode_prognosis,
            _token : "{{csrf_token()}}"
        }
        ...
        $.ajax({
            type:"POST",
            url: "{{ route('prognosis-master-kode-update') }}",
            data: param,
            success: function(response)
            {
                // console.log(response.kfa_code+ '-'+ inputElement.value)
                if(response.kode_prognosis != inputElement.value)
                {
                    inputElement.value = kode_prognosis;
