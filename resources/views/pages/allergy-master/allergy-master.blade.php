...
<div class="nk-block-head nk-block-head-sm">
    <div class="nk-block-between">
        <div class="nk-block-head-content">
            <h3 class="nk-block-title page-title">Allergy Master</h3>
...
<script type="text/javascript">
    ...
    {
        var id_input_element = event.target.name.replace('id_allergy_master_', '');


        // console.log(display_name)
        loadingProcess(); //dari custom.js
        var url     ='{{ route("allergy-master-kode",":id") }}';
    ...
    function updateKode(id_allergy_master,kode_allergy)
    {
        var inputElement = document.querySelector('input[name="id_allergy_master_'+id_allergy_master+'"]');


        var param = {
            id_allergy_master : parseInt(id_allergy_master),
            kode_allergy : kode_allergy,
            _token : "{{csrf_token()}}"
        }
        ...
        $.ajax({
            type:"POST",
            url: "{{ route('allergy-master-kode-update') }}",
            data: param,
            success: function(response)
            {
                // console.log(response.kfa_code+ '-'+ inputElement.value)
                if(response.kode_allergy != inputElement.value)
                {
                    inputElement.value = kode_allergy;
